...
function InvoicesPage({ invoices, loading, error, onSearch, onRetry, onOpen }) {
  ...
  return (
    <section className="panel data-panel invoice-history">
      ...
      {!loading && !error && invoices.length > 0 && <div className="table-wrap"><table className="data-table invoice-history-table"><thead><tr><th>HÓA ĐƠN</th><th>BÀN</th><th>THU NGÂN</th><th>PHƯƠNG THỨC</th><th>TỔNG TIỀN</th><th>THỜI GIAN</th><th>TRẠNG THÁI</th></tr></thead><tbody>{invoices.map((invoice) => <tr className="clickable-row" key={invoice.invoice_id} tabIndex="0" aria-label={`Mở hóa đơn ${invoice.invoice_id}`} onClick={() => onOpen(invoice.invoice_id)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onOpen(invoice.invoice_id); } }}><td><strong>#{String(invoice.invoice_id).padStart(6, "0")}</strong><span>Đơn #{String(invoice.order_id).padStart(4, "0")}</span></td><td>{invoice.table_number}</td><td>{invoice.cashier_name}</td><td><span className={`payment-badge ${invoice.payment_method.toLowerCase()}`}>{paymentLabels[invoice.payment_method]}</span></td><td className="price-cell">{currency(invoice.amount)}</td><td>{formatDate(invoice.paid_at)}</td><td><span className="status-badge available"><span className="status-dot" />Đã thanh toán</span></td></tr>)}</tbody></table></div>}
    </section>
  );
}
