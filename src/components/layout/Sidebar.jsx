      ...
      <aside className={`sidebar ${isOpen ? "open" : ""}`}>
        ...
        <nav className="sidebar-nav" aria-label="Điều hướng chính">
          ...
          {navItems.filter((item) => item.roles.includes(user.role)).map((item) => {
            const isActive = currentPage === item.id || (currentPage === "order" && item.id === "tables") || (currentPage === "invoiceDetail" && item.id === "invoices");
            return <button key={item.id} className={`nav-item ${isActive ? "active" : ""}`} aria-current={isActive ? "page" : undefined} onClick={() => onNavigate(item.id)}><Icon name={item.icon} /><span>{item.label}</span></button>;
          })}
        </nav>
