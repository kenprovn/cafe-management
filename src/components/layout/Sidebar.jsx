    ...
    <>
      ...
      <aside className={`sidebar ${isOpen ? "open" : ""}`}>
        ...
        <nav className="sidebar-nav" aria-label="Điều hướng chính">
          ...
          {navItems.map((item) => <button key={item.id} className={`nav-item ${currentPage === item.id || (currentPage === "order" && item.id === "tables") || (currentPage === "invoiceDetail" && item.id === "invoices") ? "active" : ""}`} onClick={() => onNavigate(item.id)}><Icon name={item.icon} /><span>{item.label}</span></button>)}
