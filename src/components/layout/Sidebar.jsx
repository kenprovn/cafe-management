...
const comingSoonItems = [
  { label: "Báo cáo", icon: "report", roles: ["admin"] },
];
    ...
    <>
      ...
      <aside className={`sidebar ${isOpen ? "open" : ""}`}>
        ...
        <nav className="sidebar-nav" aria-label="Điều hướng chính">
          ...
          {comingSoonItems.some((item) => item.roles.includes(user.role)) && <span className="nav-label secondary-label">SẮP RA MẮT</span>}
          {comingSoonItems.filter((item) => item.roles.includes(user.role)).map((item) => <button key={item.label} className="nav-item disabled" disabled title="Tính năng đang phát triển"><Icon name={item.icon} /><span>{item.label}</span><small>Sớm</small></button>)}
        </nav>
